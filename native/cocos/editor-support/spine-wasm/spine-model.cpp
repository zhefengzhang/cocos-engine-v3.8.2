...
SpineModel::SpineModel() {
    ...
    attachedSlotNameArray = new std::vector<std::string>();
}

SpineModel::~SpineModel() {
    delete SpineModel::data; 
    SpineModel::data = nullptr;
    delete attachedSlotNameArray;
    attachedSlotNameArray = nullptr;
}
...
void SpineModel::addAttachedSlotName(const char* name) {
    attachedSlotNameArray->push_back(name);
}

void SpineModel::clearMeshes() {
    data->resize(0);
    attachedSlotNameArray->clear();
    vCount = 0;
    iCount = 0;
}
