...
void SpineSkeletonInstance::collectMeshData() {
    ...
    for (uint32_t drawIdx = 0; drawIdx < slotCount; ++drawIdx) {
        ...
        if (!slot->getAttachment()) {
            ...
            processAttachedSlot(slot->getData().getName().buffer());
            continue;
        } 
        ...
        _model->addSlotMesh(currMesh, _canMerge);
        _clipper->clipEnd(*slot);
        processAttachedSlot(slot->getData().getName().buffer(), true);
    }
...
void SpineSkeletonInstance::processAttachedSlot(const char* slotName, bool reset) {
    for (const std::string& name : _attachedSlotNames) {
        if (name.compare(slotName) == 0) {
            _canMerge = false;
            _model->addAttachedSlotName(slotName);
            return;
        }
    }
    if (reset) {
...
void SpineSkeletonInstance::setAttachedSlotNames(std::vector<std::string> &slotNames) {
    _attachedSlotNames.assign(slotNames.begin(), slotNames.end());
}
