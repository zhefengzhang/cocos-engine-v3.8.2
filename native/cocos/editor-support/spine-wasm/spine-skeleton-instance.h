...
public:
    ...
    bool enable{true};
    float dtRate{1.0F};
    const std::vector<std::string> & getAttachedSlotNames() const {return _attachedSlotNames;}
    void setAttachedSlotNames(std::vector<std::string> &slotNames);
private:
    void collectMeshData();
    void processAttachedSlot(const char* slotName, bool reset = false);
...
private:
    ...
    std::vector<std::string> _attachedSlotNames{};
};
